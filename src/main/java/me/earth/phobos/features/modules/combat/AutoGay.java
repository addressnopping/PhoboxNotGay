...
public class AutoGay
    ...
    public AutoGay() {
        super("AutoGay", "le insana crystalpogaura", Module.Category.COMBAT, true, false, false);
    }
    ...
    private void rotateTo(Entity entity) {
        if (this.rotate.getValue().booleanValue()) {
            float[] angle = MathUtil.calcAngle(AutoGay.mc.player.getPositionEyes(mc.getRenderPartialTicks()), entity.getPositionVector());
            this.yaw = angle[0];
            this.pitch = angle[1];
            this.rotating = true;
        }
    ...
    private void rotateToPos(BlockPos pos) {
        if (this.rotate.getValue().booleanValue()) {
            float[] angle = MathUtil.calcAngle(AutoGay.mc.player.getPositionEyes(mc.getRenderPartialTicks()), new Vec3d((float) pos.getX() + 0.5f, (float) pos.getY() - 0.5f, (float) pos.getZ() + 0.5f));
            this.yaw = angle[0];
            this.pitch = angle[1];
            this.rotating = true;
        }
    ...
    public void onCrystal() {
        if (AutoGay.mc.world == null || AutoGay.mc.player == null) {
            return;
        }
        this.realTarget = null;
        this.manualBreaker();
        ...
        if (!this.ignoreUseAmount.getValue().booleanValue()) {
            for (Entity crystal : AutoGay.mc.world.loadedEntityList) {
        ...
        this.hotBarSlot = -1;
        if (AutoGay.mc.player.getHeldItemOffhand().getItem() != Items.END_CRYSTAL) {
            int crystalSlot;
            crystalSlot = AutoGay.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL ? AutoGay.mc.player.inventory.currentItem : -1;
            if (crystalSlot == -1) {
                for (int l = 0; l < 9; ++l) {
                    if (AutoGay.mc.player.inventory.getStackInSlot(l).getItem() != Items.END_CRYSTAL) continue;
                    crystalSlot = l;
                    this.hotBarSlot = l;
                    break;
                }
        ...
        if (AutoGay.mc.player.getHeldItemOffhand().getItem() == Items.GOLDEN_APPLE && AutoGay.mc.player.getHeldItemMainhand().getItem() != Items.END_CRYSTAL) {
            this.pos = null;
            this.target = null;
            return;
        }
        ...
        if (this.target.getDistance(AutoGay.mc.player) > 12.0f) {
            this.crystal = null;
            this.target = null;
        }
        this.crystal = AutoGay.mc.world.loadedEntityList.stream().filter(this::IsValidCrystal).map(p_Entity -> (EntityEnderCrystal) p_Entity).min(Comparator.comparing(p_Entity -> Float.valueOf(this.target.getDistance((Entity) p_Entity)))).orElse(null);
        if (this.crystal != null && this.explode.getValue().booleanValue() && this.breakTimer.passedMs(this.breakDelay.getValue().longValue())) {
            ...
            if (this.packetBreak.getValue().booleanValue()) {
                ...
                AutoGay.mc.player.connection.sendPacket(new CPacketUseEntity(this.crystal));
            } else {
                this.rotateTo(this.crystal);
                AutoGay.mc.playerController.attackEntity(AutoGay.mc.player, this.crystal);
            }
            if (this.swingMode.getValue() == SwingMode.MainHand) {
                AutoGay.mc.player.swingArm(EnumHand.MAIN_HAND);
            } else if (this.swingMode.getValue() == SwingMode.OffHand) {
                AutoGay.mc.player.swingArm(EnumHand.OFF_HAND);
            }
        }
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue().booleanValue()) {
            ...
            for (BlockPos blockPos : this.placePostions(this.placeRange.getValue().floatValue())) {
                ...
                if (targetDmg < (double) this.minDamage.getValue().floatValue() && (this.facePlaceSword.getValue() != false ? this.target.getAbsorptionAmount() + this.target.getHealth() > this.facePlace.getValue().floatValue() : AutoGay.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword || this.target.getAbsorptionAmount() + this.target.getHealth() > this.facePlace.getValue().floatValue()) && (this.facePlaceSword.getValue() != false ? !this.armor : AutoGay.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword || !this.armor) || (selfDmg = this.calculateDamage((double) blockPos.getX() + 0.5, (double) blockPos.getY() + 1.0, (double) blockPos.getZ() + 0.5, AutoGay.mc.player)) + (this.suicide.getValue() != false ? 2.0 : 0.5) >= (double) (AutoGay.mc.player.getHealth() + AutoGay.mc.player.getAbsorptionAmount()) && selfDmg >= targetDmg && targetDmg < (double) (this.target.getHealth() + this.target.getAbsorptionAmount()) || !(damage < targetDmg))
                    continue;
                this.pos = blockPos;
                damage = targetDmg;
            }
          ...
          /*
            else if (this.hotBarSlot != -1 && this.autoswitch.getValue().booleanValue() && !AutoGay.mc.player.isPotionActive(MobEffects.WEAKNESS) && this.silent.getValue().booleanValue() == false) {
                AutoGay.mc.player.inventory.currentItem = this.hotBarSlot;
            }
          */
            if (!this.ignoreUseAmount.getValue().booleanValue()) {
                ...
                if (this.crystalCount < crystalLimit && this.pos != null) {
                    ...
                    AutoGay.mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(this.pos, EnumFacing.UP, AutoGay.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
                }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof SPacketSpawnObject && (packet = event.getPacket()).getType() == 51 && this.predicts.getValue().booleanValue() && this.preditTimer.passedMs(this.attackFactor.getValue().longValue()) && this.predicts.getValue().booleanValue() && this.explode.getValue().booleanValue() && this.packetBreak.getValue().booleanValue() && this.target != null) {
            ...
            AutoGay.mc.player.connection.sendPacket(predict);
        }
    }
    ...
    private boolean isPredicting(SPacketSpawnObject packet) {
        ...
        if (AutoGay.mc.player.getDistance(packet.getX(), packet.getY(), packet.getZ()) > (double) this.breakRange.getValue().floatValue()) {
            return false;
        }
        if (!this.canSeePos(packPos) && AutoGay.mc.player.getDistance(packet.getX(), packet.getY(), packet.getZ()) > (double) this.breakWallRange.getValue().floatValue()) {
            return false;
        }
        double targetDmg = this.calculateDamage(packet.getX() + 0.5, packet.getY() + 1.0, packet.getZ() + 0.5, this.target);
        if (EntityUtil.isInHole(AutoGay.mc.player) && targetDmg >= 1.0) {
            return true;
        }
        double selfDmg = this.calculateDamage(packet.getX() + 0.5, packet.getY() + 1.0, packet.getZ() + 0.5, AutoGay.mc.player);
        double d = this.suicide.getValue() != false ? 2.0 : 0.5;
        if (selfDmg + d < (double) (AutoGay.mc.player.getHealth() + AutoGay.mc.player.getAbsorptionAmount()) && targetDmg >= (double) (this.target.getAbsorptionAmount() + this.target.getHealth())) {
            return true;
        }
        this.armorTarget = false;
        for (ItemStack is : this.target.getArmorInventoryList()) {
    ...
    private boolean IsValidCrystal(Entity p_Entity) {
        ...
        if (p_Entity.getDistance(AutoGay.mc.player) > this.breakRange.getValue().floatValue()) {
            return false;
        }
        if (!AutoGay.mc.player.canEntityBeSeen(p_Entity) && p_Entity.getDistance(AutoGay.mc.player) > this.breakWallRange.getValue().floatValue()) {
            return false;
        }
        ...
        double selfDmg = this.calculateDamage((double) p_Entity.getPosition().getX() + 0.5, (double) p_Entity.getPosition().getY() + 1.0, (double) p_Entity.getPosition().getZ() + 0.5, AutoGay.mc.player);
        double d = this.suicide.getValue() != false ? 2.0 : 0.5;
        if (selfDmg + d < (double) (AutoGay.mc.player.getHealth() + AutoGay.mc.player.getAbsorptionAmount()) && targetDmg >= (double) (this.target.getAbsorptionAmount() + this.target.getHealth())) {
            return true;
        }
    ...
    EntityPlayer getTarget() {
        ...
        for (EntityPlayer entity : AutoGay.mc.world.playerEntities) {
            if (AutoGay.mc.player == null || AutoGay.mc.player.isDead || entity.isDead || entity == AutoGay.mc.player || Phobos.friendManager.isFriend(entity.getName()) || entity.getDistance(AutoGay.mc.player) > 12.0f)
                continue;
            this.armorTarget = false;
            ...
            if (!(closestPlayer.getDistance(AutoGay.mc.player) > entity.getDistance(AutoGay.mc.player)))
                continue;
            closestPlayer = entity;
        }
    ...
    private void manualBreaker() {
        ...
        if (this.manualTimer.passedMs(200L) && AutoGay.mc.gameSettings.keyBindUseItem.isKeyDown() && AutoGay.mc.player.getHeldItemOffhand().getItem() != Items.GOLDEN_APPLE && AutoGay.mc.player.inventory.getCurrentItem().getItem() != Items.GOLDEN_APPLE && AutoGay.mc.player.inventory.getCurrentItem().getItem() != Items.BOW && AutoGay.mc.player.inventory.getCurrentItem().getItem() != Items.EXPERIENCE_BOTTLE && (result = AutoGay.mc.objectMouseOver) != null) {
            if (result.typeOfHit.equals(RayTraceResult.Type.ENTITY)) {
                Entity entity = result.entityHit;
                if (entity instanceof EntityEnderCrystal) {
                    if (this.packetBreak.getValue().booleanValue()) {
                        AutoGay.mc.player.connection.sendPacket(new CPacketUseEntity(entity));
                    } else {
                        AutoGay.mc.playerController.attackEntity(AutoGay.mc.player, entity);
                    }
                    this.manualTimer.reset();
                }
            } else if (result.typeOfHit.equals(RayTraceResult.Type.BLOCK)) {
                BlockPos mousePos = new BlockPos(AutoGay.mc.objectMouseOver.getBlockPos().getX(), (double) AutoGay.mc.objectMouseOver.getBlockPos().getY() + 1.0, AutoGay.mc.objectMouseOver.getBlockPos().getZ());
                for (Entity target : AutoGay.mc.world.getEntitiesWithinAABBExcludingEntity(null, new AxisAlignedBB(mousePos))) {
                    if (!(target instanceof EntityEnderCrystal)) continue;
                    if (this.packetBreak.getValue().booleanValue()) {
                        AutoGay.mc.player.connection.sendPacket(new CPacketUseEntity(target));
                    } else {
                        AutoGay.mc.playerController.attackEntity(AutoGay.mc.player, target);
                    }
                    this.manualTimer.reset();
                }
            }
    ...
    private boolean canSeePos(BlockPos pos) {
        return AutoGay.mc.world.rayTraceBlocks(new Vec3d(AutoGay.mc.player.posX, AutoGay.mc.player.posY + (double) AutoGay.mc.player.getEyeHeight(), AutoGay.mc.player.posZ), new Vec3d(pos.getX(), pos.getY(), pos.getZ()), false, true, false) == null;
    }

    private NonNullList<BlockPos> placePostions(float placeRange) {
        NonNullList positions = NonNullList.create();
        positions.addAll(AutoGay.getSphere(new BlockPos(Math.floor(AutoGay.mc.player.posX), Math.floor(AutoGay.mc.player.posY), Math.floor(AutoGay.mc.player.posZ)), placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> this.canPlaceCrystal(pos, true)).collect(Collectors.toList()));
        return positions;
    }
    ...
    private boolean canPlaceCrystal(BlockPos blockPos, boolean specialEntityCheck) {
        ...
        try {
            if (!this.opPlace.getValue().booleanValue()) {
                if (AutoGay.mc.world.getBlockState(blockPos).getBlock() != Blocks.BEDROCK && AutoGay.mc.world.getBlockState(blockPos).getBlock() != Blocks.OBSIDIAN) {
                    return false;
                }
                if (AutoGay.mc.world.getBlockState(boost).getBlock() != Blocks.AIR || AutoGay.mc.world.getBlockState(boost2).getBlock() != Blocks.AIR) {
                    return false;
                }
                if (!specialEntityCheck) {
                    return AutoGay.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost)).isEmpty() && AutoGay.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost2)).isEmpty();
                }
                for (Entity entity : AutoGay.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost))) {
                    if (entity instanceof EntityEnderCrystal) continue;
                    return false;
                }
                for (Entity entity : AutoGay.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost2))) {
                    if (entity instanceof EntityEnderCrystal) continue;
                    return false;
                }
            } else {
                ...
                if (AutoGay.mc.world.getBlockState(boost).getBlock() != Blocks.AIR) {
                    return false;
                }
                if (!specialEntityCheck) {
                    return AutoGay.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost)).isEmpty();
                }
                for (Entity entity : AutoGay.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost))) {
                    if (entity instanceof EntityEnderCrystal) continue;
                    return false;
    ...
    private float calculateDamage(double posX, double posY, double posZ, Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            finald = this.getBlastReduction((EntityLivingBase) entity, this.getDamageMultiplied(damage), new Explosion(AutoGay.mc.world, null, posX, posY, posZ, 6.0f, false, true));
        }
        return (float) finald;
    }
    ...
    private float getDamageMultiplied(float damage) {
        int diff = AutoGay.mc.world.getDifficulty().getId();
